...
// node_modules/@giro3d/giro3d/utils/ProjUtils.js
...
function transformBufferInPlace(buf, params) {
  ...
  if (params.stride === void 0 || params.stride < 2) {
    throw new Error("invalid stride: must be at least 2");
  }
  const src = lib_default.Proj(srcCrsId);
  const dst = lib_default.Proj(dstCrsId);
  const tmp = {
    x: 0,
    y: 0
  };
  ...
  for (let i = 0; i < length; i += stride) {
    tmp.x = buf[i + 0];
    tmp.y = buf[i + 1];
    const out = lib_default.transform(src, dst, tmp, true);
    if (out == null) {
...
var ProjUtils_default = {
  transformBufferInPlace,
  transformVectors
};

// node_modules/@giro3d/giro3d/core/geographic/Extent.js
var tmpXY = new Vector2();
var CARDINAL = {
  WEST: 0,
  EAST: 1,
  SOUTH: 2,
...
function reasonnableEpsilonForCRS(crs, width, height) {
  if (crs.isEpsg(4326)) {
    return 0.01;
  }
  return 0.01 * Math.min(width, height);
}
var cardinals = [new Vector2(), new Vector2(), new Vector2(), new Vector2(), new Vector2(), new Vector2(), new Vector2(), new Vector2()];
var Extent = class _Extent {
  ...
  static fromCenterAndSize(crs, center, width, height) {
    ...
    return new _Extent(crs, minX, maxX, minY, maxY);
  }
  get values() {
    return this._values;
  }
  ...
  sampleUV(u, v, target) {
    ...
    if (target != null) {
      ...
      return new Coordinates_default(this._crs, x, y);
    }
  }
  /**
  ...
  clone() {
    ...
    const result = new _Extent(this._crs, minx, maxx, miny, maxy);
    return result;
  }
  /**
  ...
  withMargin(x, y) {
    ...
    return new _Extent(this.crs, w, e, s, n);
  }
  /**
  ...
  as(crs) {
    if (!this._crs.equals(crs) && !(this._crs.isEpsg(4326) && crs.isEpsg(4326))) {
      ...
      let east = -Infinity;
      let west = Infinity;
      ProjUtils_default.transformVectors(this._crs, crs, cardinals);
  ...
  offsetToParent(other, target = new OffsetScale()) {
    ...
    const originX = Math.round(1e3 * (this.west - other.west) / oDim.x) * 1e-3;
    const originY = Math.round(1e3 * (this.south - other.south) / oDim.y) * 1e-3;
    const scaleX = Math.round(1e3 * dim.x / oDim.x) * 1e-3;
    const scaleY = Math.round(1e3 * dim.y / oDim.y) * 1e-3;
   ...
   */
  topLeft() {
    return new Coordinates_default(this.crs, this.west, this.north, 0);
  }
  /**
   * @returns the coordinates of the top right corner
   */
  topRight() {
    return new Coordinates_default(this.crs, this.east, this.north, 0);
  }
  /**
   * @returns the coordinates of the bottom right corner
   */
  bottomRight() {
    return new Coordinates_default(this.crs, this.east, this.south, 0);
  }
  /**
   * @returns the coordinates of the bottom right corner
   */
  bottomLeft() {
    return new Coordinates_default(this.crs, this.west, this.south, 0);
  }
  /**
  ...
  center(target) {
    ...
    if (target) {
      ...
      result = new Coordinates_default(this._crs, center.x, center.y, 0);
    }
    return result;
  }
  /**
  ...
  fitToGrid(gridExtent, gridWidth, gridHeight, minPixWidth, minPixHeight) {
    ...
    if (minPixWidth !== void 0 && minPixHeight !== void 0) {
    ...
    return {
      extent: new _Extent(this.crs, west, east, south, north),
  ...
  static unionMany(...extents) {
    ...
    if (valid) {
      return new _Extent(extents[0].crs, west, east, south, north);
    } else {
      return null;
    }
  }
  ...
  split(xSubdivs, ySubdivs) {
    ...
    for (let x = 0; x < xSubdivs; x++) {
      for (let y = 0; y < ySubdivs; y++) {
        ...
        const extent = new _Extent(crs, west, west + w, south, south + h);
        result.push(extent);
      }
    }
    return result;
  ...
  static get webMercator() {
    return new _Extent(CoordinateSystem.epsg3857, -2003750834e-2, 2003750834e-2, -200489661e-1, 200489661e-1);
  }
  /**
   * The bounds of the whole world in the EPSG:4326 projection.
   */
  static get WGS84() {
    return new _Extent(CoordinateSystem.epsg4326, -180, 180, -90, 90);
  }
  /**
   * The bounds of the whole sphere in the `'equirectangular'` projection.
   */
  static get fullEquirectangularProjection() {
    return new _Extent(CoordinateSystem.equirectangular, -180, 180, -90, 90);
  }
  /**
  ...
  static fromPhotosphere(params) {
    if (params == null) {
      return _Extent.fullEquirectangularProjection;
    }
    ...
    return new _Extent(CoordinateSystem.equirectangular, {
      west,
      east,
      south,
      north
...
export {
  ProjUtils_default,
  reasonnableEpsilonForCRS,
  Extent_default,
  isCoordinates
};
//# sourceMappingURL=chunk-RJV5WS67.js.map
