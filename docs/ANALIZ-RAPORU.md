...
Royal Cabana, plaj kulübesi (Cabana) rezervasyon yönetimi için tasarlanmış fullstack bir Next.js uygulaması.
...
| `CabanaClass`              | Cabana sınıfları (VIP, Standard vb.), attributes ile genişletilebilir         |
...
| `CabanaPrice`              | Cabana bazlı günlük dinamik fiyat (cabanaId + date unique)                    |
...
| `ModificationRequest`      | Değişiklik talepleri (yeni Cabana, tarih, misafir adı)                        |
...
| Öncelik | Sorun                                | Açıklama                                                                                                                                                                                                        |
| ------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Kritik  | Race Condition — Çakışma Kontrolü    | Rezervasyon oluşturma (`POST /api/reservations`) çakışma kontrolü ile kayıt oluşturma arasında transaction yok. İki eşzamanlı istek aynı Cabana+tarih için onaylanabilir. `findFirst` + `create` atomik değil.  |
...
| Yüksek  | Modification atomikliği              | `ModificationRequest` approve edildiğinde yeni Cabana/tarih için çakışma kontrolü yapılıp yapılmadığı doğrulanmalı. Modification approve + reservation update + cabana status update tek transaction'da olmalı. |
| Orta    | Cancellation sonrası Cabana status   | İptal onaylandığında `Cabana.status` otomatik olarak `AVAILABLE`'a dönmüyor olabilir. Aynı Cabana için başka aktif rezervasyon varsa status yönetimi karmaşıklaşır.                                             |
...
1. **Race Condition — Rezervasyon Çakışma Kontrolü**: Çakışma kontrolü ile kayıt oluşturma atomik değil. İki eşzamanlı istek aynı Cabana+tarih için başarılı olabilir. Pessimistic locking veya serializable transaction gerekli.
...
const reservation = await prisma.$transaction(async (tx) => {
  // Pessimistic lock: aynı Cabana için çakışan kayıtları kilitle
