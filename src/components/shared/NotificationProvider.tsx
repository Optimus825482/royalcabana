...
const EVENT_MAP: Record<string, EventMapping> = {
  ...
  [SSE_EVENTS.RESERVATION_UPDATED]: {
    ...
    sound: "info",
    title: (d) =>
      `Talep Güncellendi${d.cabanaName ? ` — ${d.cabanaName}` : ""}`,
    message: (d) =>
      d.guestName
        ? `${d.guestName} adına talep güncellendi.`
        : "Bir rezervasyon talebi güncellendi.",
  },
  [SSE_EVENTS.RESERVATION_CHECKED_IN]: {
    notifType: "reservation_checked_in",
    sound: "success",
    title: (d) => `Check-in Yapıldı${d.cabanaName ? ` — ${d.cabanaName}` : ""}`,
    message: (d) =>
      d.guestName
  ...
  [SSE_EVENTS.CABANA_STATUS_CHANGED]: {
    ...
    sound: "warning",
    title: (d) =>
      `Cabana Durumu Değişti${d.cabanaName ? ` — ${d.cabanaName}` : ""}`,
    message: (d) =>
      d.status
        ? `Cabana durumu: ${d.status}`
