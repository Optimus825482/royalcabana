...
export default function SystemAdminReservationsPage() {
  ...
  // Status counts — we don't have server counts yet, use total
  return (
    <div className="text-neutral-100 flex flex-col h-full">
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-neutral-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {message && (
            <div
              className={`text-sm px-4 py-2 rounded-lg animate-in fade-in ${
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left min-h-[52px] px-4 sm:px-6 py-3.5 border-b border-neutral-800/50 hover:bg-neutral-800/50 active:bg-neutral-800/70 transition-colors touch-manipulation focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50 ${
                      selected?.id === r.id ? "bg-amber-500/10 border-l-2 border-l-amber-500 sm:border-l-0 sm:ring-2 sm:ring-inset sm:ring-amber-500/40" : ""
                    }`}
                  >
                    {/* Mobile layout */}
      ...
      {/* Detay modal */}
      <Dialog open={detailLoading || !!selected} onOpenChange={(open) => !open && closeDetailModal()}>
        <DialogContent
