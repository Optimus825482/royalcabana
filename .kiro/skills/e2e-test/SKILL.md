...
- `MINGW`, `CYGWIN`, or native Windows → check for WSL:

```bash
# On Windows, check if WSL is available
wsl --list --verbose 2>/dev/null
```

**WSL Routing (Windows only):**
If running on native Windows but WSL is detected:

1. All `agent-browser` commands MUST be routed through WSL: `wsl -e agent-browser <args>`
2. Ensure agent-browser is installed INSIDE WSL, not on Windows
3. File paths must be converted: Windows `D:\project` → WSL `/mnt/d/project`
4. The dev server should be started on Windows (accessible via localhost), but agent-browser runs in WSL

If neither Linux/macOS nor WSL is available:

> "agent-browser only supports Linux, WSL, and macOS. It cannot run on native Windows. Please install WSL with `wsl --install` or run from a Linux/macOS environment."

Stop execution if the platform is unsupported and WSL is not available.
...
Check if agent-browser is installed (use `wsl -e` prefix on Windows):
...
> Research this codebase's database layer. Read `.env.example` to understand environment variables for database connections. DO NOT read `.env` or `.env.local` directly — these contain real secrets. Return a structured summary covering:
>
